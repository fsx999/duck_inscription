...
class SettingsEtape(Etape):
    ...
    def stat_suivi_dossier(self):
        ...
        return dict(WishTransitionLog.objects.filter(
            wish__etape=self, wish__is_auditeur=False).values_list('to_state').annotate(Count('to_state')))

    def stat_etat_dossier(self):
        return dict(self.wish_set.filter(is_auditeur=False).values_list('suivi_dossier').annotate(Count('suivi_dossier')))

    def stat_nb_reception(self):
        from duck_inscription.models import WishTransitionLog
        return WishTransitionLog.objects.filter(wish__is_auditeur=False, wish__etape=self,
                                                to_state='inscription_reception').distinct('wish').count()
