...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        t = ContentType.objects.get(model='individu', app_label='duck_inscription')

        for trans in TransitionLog.objects.filter(content_type=t):
            try:
                IndividuTransitionLog.objects.create(
                    transition=trans.transition,
                    from_state=trans.from_state,
                    to_state=trans.to_state,
                    timestamp=trans.timestamp,
                    individu_id=trans.content_id
                )
            except IntegrityError:
                pass
        print IndividuTransitionLog.objects.all().count()
